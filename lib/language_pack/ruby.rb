...
private
  ...
  def default_path
    "/app/bin:bin:#{slug_vendor_base}/bin:/usr/local/bin:/usr/bin:/bin"
  end
  ...
  def build_bundler
    log("bundle") do
      ...
      Dir.mktmpdir("libyaml-") do |tmpdir|
        ...
        puts run("#{env_vars} bundle config build.sqlite3-ruby --with-opt-dir=#{pwd}/lib 2>&1")
        puts run("#{env_vars} bundle config sqlite3-ruby --with-opt-dir=#{pwd}/lib 2>&1")
        puts run("#{env_vars} bundle config build.sqlite3 --with-opt-dir=#{pwd}/lib 2>&1")
